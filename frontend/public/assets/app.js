(() => {
  ...
  const PAGE_COPY = {
    ...
    'sanctions-news': ['制裁与负面新闻审查', '检索制裁名单、负面媒体、监管行动、诉讼与受益所有权信息。'],
    'market-funding': ['市场与资金环境分析', '分析利率、汇率、商品价格和融资条件对基础设施融资的影响。'],
  ...
  function openPlanDetail(id) {
    ...
    const frozen = plan.status === 'frozen';
    const pdt = $('#plan-drawer-title'); if (pdt) pdt.textContent = `${plan.userPlan.symbol} · ${plan.version}`;
    const pdc = $('#plan-drawer-content'); if (!pdc) return;
    pdc.innerHTML = `<div class="plan-detail-hero"><div><span class="status-chip ${frozen ? 'neutral' : 'partial'}">${frozen ? '已冻结 · 待重评' : '模拟草稿 · 待复核'}</span><h3>${escapeHtml(plan.userPlan.symbol)}</h3><p>${escapeHtml(plan.userPlan.thesis || '分阶段研究计划')}</p></div><div class="version-stamp"><span>证据版本</span><strong>${escapeHtml(plan.version)}</strong><small>${escapeHtml(new Date(plan.createdAt).toLocaleString('zh-CN'))}</small></div></div>
